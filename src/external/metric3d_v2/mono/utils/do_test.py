...
def get_prediction(
    ...
    # output_dict: 'prediction', 'predictions_list', 'confidence', 'confidence_list', 'pred_logit', 'prediction_normal', 'normal_out_list', 'low_resolution_init'

    for i in range(len(output_dict['predictions_list'])):
        print(output_dict['predictions_list'][i].shape)
    assert False
    ...
    for i in tqdm(range(0, len(test_data), bs)):
        ...
        for an in batch_data:
            ...
            if an['depth'] is not None:
                ...
                gt_depth = gt_depth / gt_depth_scale  #NOTE: check this
            gt_depths.append(gt_depth)
            
            intrinsic = an['intrinsic']
            if intrinsic is None:
                intrinsic = [1000.0, 1000.0, rgb_origin.shape[1]/2, rgb_origin.shape[0]/2] #NOTE: check this
