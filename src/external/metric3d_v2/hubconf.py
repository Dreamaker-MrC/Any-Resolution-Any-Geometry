...
if __name__ == '__main__':
  ...
  import numpy as np
  #### prepare data
  rgb_file = '8k_samples/jars_2k.jpg'
  rgb_name = rgb_file.split('/')[-1].split('.')[0]
  depth_file = None
  ...
  if 'prediction_normal' in output_dict: # only available for Metric3Dv2, i.e. vit model
    ...
    pred_normal = torch.nn.functional.interpolate(pred_normal.unsqueeze(0), size=rgb_origin.shape[:2], mode='bilinear').squeeze(0)
  
    # you can now do anything with the normal
    # such as visualize pred_normal
    pred_normal_vis = pred_normal.cpu().numpy().transpose((1, 2, 0))
    np.save(f'8k_samples/{rgb_name}_coarse_normal.npy', pred_normal_vis)
    pred_normal_vis = (pred_normal_vis + 1) / 2
    cv2.imwrite(f'8k_samples/{rgb_name}_coarse_normal_vis.png', (pred_normal_vis * 255).astype(np.uint8))